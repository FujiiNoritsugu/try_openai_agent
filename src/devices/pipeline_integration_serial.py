"""
パイプライン統合モジュール（シリアル通信版）

このモジュールは、感情分析パイプラインと触覚フィードバックデバイスを統合し、
感情分析の結果に基づいて適切な振動パターンをデバイスにシリアル通信で送信するための
インターフェースを提供します。
"""
...
from .serial_controller import SerialController, SerialControllerManager
...
class HapticFeedbackIntegration:
    ...
    シリアルコントローラーを通じて適切な振動パターンを
    触覚フィードバックデバイスに送信します。
    """

    def __init__(self, serial_manager: Optional[SerialControllerManager] = None):
        """
        HapticFeedbackIntegrationを初期化します。

        引数:
            serial_manager: シリアルコントローラーマネージャー。
                          指定しない場合は新しいインスタンスが作成されます。
        """
        self.logger = logging.getLogger(__name__)
        self.serial_manager = serial_manager or SerialControllerManager()
    ...
    async def initialize(self, device_configs: List[Dict[str, Any]]) -> bool:
        ...
        引数:
            device_configs: デバイス設定のリスト。各設定は以下のキーを含む辞書:
                - device_id: デバイスの識別子
                - port: シリアルポート（オプション、自動検出可能）
                - baudrate: ボーレート（オプション、デフォルト: 115200）
        ...
        self.logger.info(
            f"{len(device_configs)}台のデバイスで触覚フィードバックシステムを初期化中"
        )
        ...
        try:
            for config in device_configs:
                ...
                self.serial_manager.register_controller(
                    device_id, port, baudrate
                )
                self.connected_devices.add(device_id)

            if self.connected_devices:
                connection_results = await self.serial_manager.connect_all()
                ...
                for device_id, success in connection_results.items():
                    if success:
                        ...
                        self.logger.warning(
                            f"デバイス '{device_id}' への接続に失敗しました"
                        )
                        self.connected_devices.discard(device_id)

                self.is_initialized = True
                self.logger.info(
                    f"触覚フィードバックシステムが初期化されました（接続済みデバイス: {len(self.connected_devices)}）"
                )
            ...
            self.logger.error(
                f"触覚フィードバックシステムの初期化中にエラーが発生しました: {str(e)}"
            )
            return False
    ...
    async def shutdown(self) -> bool:
        ...
        try:
            await self.serial_manager.stop_all()

            disconnect_results = await self.serial_manager.disconnect_all()
            ...
            self.logger.error(
                f"触覚フィードバックシステムのシャットダウン中にエラーが発生しました: {str(e)}"
            )
            return False
    ...
    async def process_pipeline_result(self, ctx: PipelineContext) -> Dict[str, bool]:
        ...
        self.logger.info(
            f"パイプライン結果を処理中: カテゴリ={ctx.emotion_category}, 感情={ctx.emotion}"
        )

        try:
            results = await self.serial_manager.process_pipeline_context(ctx)

            for device_id, success in results.items():
                if success:
                    self.logger.info(
                        f"デバイス '{device_id}' にパターンを正常に送信しました"
                    )
                else:
                    self.logger.warning(
                        f"デバイス '{device_id}' へのパターン送信に失敗しました"
                    )
            ...
            self.logger.error(
                f"パイプライン結果の処理中にエラーが発生しました: {str(e)}"
            )
            return {}

    async def run_pipeline_and_send(
        self, user_input, emotion_learner=None
        ...
        try:
            ...
            if error:
                self.logger.error(
                    f"パイプライン実行中にエラーが発生しました: {str(error)}"
                )
                return {}, {}
            ...
            self.logger.error(
                f"パイプライン実行と送信中にエラーが発生しました: {str(e)}"
            )
            return {}, {}
    ...
    async def stop_all_devices(self) -> Dict[str, bool]:
        ...
        try:
            results = await self.serial_manager.stop_all()

            for device_id, success in results.items():
                if success:
                    self.logger.info(
                        f"デバイス '{device_id}' の振動を正常に停止しました"
                    )
                else:
                    self.logger.warning(
                        f"デバイス '{device_id}' の振動停止に失敗しました"
                    )
    ...
    async def get_all_device_status(self) -> Dict[str, Any]:
        ...
        try:
            return await self.serial_manager.get_all_status()
