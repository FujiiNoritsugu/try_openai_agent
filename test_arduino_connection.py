...
async def test_stress_patterns(controller: ArduinoController) -> bool:
    ...
    try:
        ...
        complex_pattern = VibrationPattern(
            steps=[
                VibrationStep(intensity=0.5, duration_ms=100),
                VibrationStep(intensity=0.6, duration_ms=150),
                VibrationStep(intensity=0.7, duration_ms=200),
    ...
    while True:
        try:
            ...
            if command == "q":
                ...
                pattern = VibrationPattern(
                    steps=[VibrationStep(intensity=0.5, duration_ms=500)],
                    interval_ms=0,
                    repeat_count=1,
                )
                ...
                pattern = VibrationPattern(
                    steps=[VibrationStep(intensity=0.7, duration_ms=500)],
                    interval_ms=0,
                    repeat_count=1,
                )
                await controller.send_pattern(pattern)
...
async def main():
    ...
    # ArduinoのIPアドレスを取得
    arduino_ip = input(
        "ArduinoのIPアドレスを入力してください (例: 192.168.1.100): "
    ).strip()
    if not arduino_ip:
    ...
    config = ArduinoControllerConfig(
        host=arduino_ip, port=80, timeout=10.0, retry_count=3, retry_delay=1.0
    )
